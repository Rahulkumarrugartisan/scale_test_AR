...
async function iOS_setMainTexture(rug_type, material_type, modeviewer, material, textureURL) {
    if (rug_type === "handtufted") {
        ...
        if (material_type === "tibetan_wool" || material_type === "new_zealand_wool" || material_type === "pure_wool") {
            ...
            const normal_texture = await modeviewer.createTexture("Defalut_Maps/wool/New_iOS_nor.png");

            const roughtness_texture = await modeviewer.createTexture("Defalut_Maps/wool/New_iOS_mr.png");

            const oc_texture = await modeviewer.createTexture("Defalut_Maps/wool/New_iOS_oc.png");

            if (material.pbrMetallicRoughness) {
                material.pbrMetallicRoughness.setMetallicFactor(.25);
                material.pbrMetallicRoughness.setRoughnessFactor(.9);
                // material.pbrMetallicRoughness.setBaseColorFactor([3, 3, 3]);

                if ('baseColorTexture'.includes('base') || 'baseColorTexture'.includes('metallic')) {
                    material.pbrMetallicRoughness['baseColorTexture'].setTexture(texture);
                } else {
                    material['baseColorTexture'].setTexture(texture);
                }
                material.pbrMetallicRoughness.baseColorTexture.setTexture(texture);
            ...
            const normal_texture = await modeviewer.createTexture("Defalut_Maps/silk/silk_ios_nor.png");

            const roughtness_texture = await modeviewer.createTexture("Defalut_Maps/silk/silk_ios_mr.png");

            const oc_texture = await modeviewer.createTexture("Defalut_Maps/silk/silk_ios_oc.png");

            if (material.pbrMetallicRoughness) {
                material.pbrMetallicRoughness.setMetallicFactor(.5);
                material.pbrMetallicRoughness.setRoughnessFactor(.9);
                // material.pbrMetallicRoughness.setBaseColorFactor([3, 3, 3]);
...
async function get_set_backTexture(rug_type, modelviewer, material, textureURL, shape_param) {
    if (rug_type === "handtufted") {
        ...
        if (shape_param !== "arch" && shape_param !== "capsule" && shape_param !== "drop" && shape_param !== "ogee" && shape_param !== "oval" && shape_param !== "diamond" && shape_param != "splash" && shape_param != "hexagon" && shape_param != "round" && shape_param != "eight" && shape_param != "oblong" && shape_param != "splash" && shape_param != "halfmoon") {
...
*--|----------------------------------------------------------|*/
async function scale_change(parameter, user_width, user_length, rugtype, mode_viewer) {
    mode_viewer.scale = `${1} ${1} ${1}`;
    if (mode_viewer.loaded) {
        mode_viewer.scale = `${1} ${1} ${1}`;
         console.log("Model is getting updated ", " u_w : ",user_width," u_l : ",user_length, " z : ",mode_viewer.getDimensions().z," x : ",mode_viewer.getDimensions().x);
        set_rug_scale_size(parameter, user_width, user_length, mode_viewer.getDimensions().z, mode_viewer.getDimensions().x);
        ...
        //mode_viewer.scale = `${1} ${1} ${1}`;
        console.debug("updated scale before scaling: ", " length: ", u_length, " width: ", u_width, "rug type: ",rugtype);
        if (rugtype === "handknotted" || rugtype === "flatweave") {
            modelViewer1.scale = `${u_length} ${.1} ${u_width}`;
            console.debug("set scale for hand knotted");
        }
    ...
    if (is_ARmode === true) {
        if (size_para_format === "ft") {
            ...
            modelViewer.querySelector('button[slot="hotspot-dim+X-Y"]').textContent =
                `${(temp_val_width * 3.280839895).toFixed(1)} ${size_para_format}`;
            ...
            modelViewer.querySelector('button[slot="hotspot-dim+X-Z"]').textContent =
                `${(size.y * 3.280839895).toFixed(1)} ${size_para_format}`;
            ...
            modelViewer.querySelector('button[slot="hotspot-dim+Y-Z"]').textContent =
                `${(temp_val_length * 3.280839895).toFixed(1)} ${size_para_format}`;
        ...
        if (size_para_format === "cm") {
            ...
            modelViewer.querySelector('button[slot="hotspot-dim+X-Y"]').textContent =
                `${(temp_val_width * 100).toFixed(1)} ${size_para_format}`;
            ...
            modelViewer.querySelector('button[slot="hotspot-dim+X-Z"]').textContent =
                `${(size.y * 100).toFixed(1)} ${size_para_format}`;
            ...
            modelViewer.querySelector('button[slot="hotspot-dim+Y-Z"]').textContent =
                `${(temp_val_length * 100).toFixed(1)} ${size_para_format}`;
        ...
        if (size_para_format === "ft") {
            ...
            modelViewer.querySelector('button[slot="hotspot-dim+X-Y"]').textContent =
                `${(size.z * 3.280839895).toFixed(1)} ${size_para_format}`;
            ...
            modelViewer.querySelector('button[slot="hotspot-dim+Y-Z"]').textContent =
                `${(size.x * 3.280839895).toFixed(1)} ${size_para_format}`;
        ...
        if (size_para_format === "cm") {
            ...
            modelViewer.querySelector('button[slot="hotspot-dim+X-Y"]').textContent =
                `${(size.z * 100).toFixed(1)} ${size_para_format}`;
            ...
            modelViewer.querySelector('button[slot="hotspot-dim+Y-Z"]').textContent =
                `${(size.x * 100).toFixed(1)} ${size_para_format}`;
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    modelViewerTexture1.addEventListener("load", async () => {
        ...
        *--|-----------------------------------------------| */
        ...
        const totalSteps = 3; // Number of steps for progress tracking
        let currentStep = 0;
        ...
        try {
            ...
            /**const createAndApplyTexture = async (channel, event) => {
                const texture = await modelViewerTexture1.createTexture(event.target.value.toString());
                console.debug("nm map path:: ", event.target.value);
                if (channel.includes('base') || channel.includes('metallic')) {
                    material.pbrMetallicRoughness[channel].setTexture(texture);
                } else {
                    material[channel].setTexture(texture);
            ...
            //nor_m_texture = await createTiledTexture("Defalut_Maps/wool/wool_nor_n.png", 3, 3);
            // updateProgress(++currentStep, totalSteps);
         ...
         * |---------------------------------------------------------------|*/
        updateProgress(++currentStep, totalSteps);
        await scale_change(size, with_size, length_size, rugType,modelViewerTexture1);
