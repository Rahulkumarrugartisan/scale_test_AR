...
async function scale_change(parameter, user_width, user_length, rugtype) {
    ...
    if (modelViewer1.loaded) {
        ...
        if (rugType === "handtufted") {
            modelViewer.scale = `${u_length} ${1} ${u_width}`;
        }
        else {
            modelViewer.scale = `${u_length} ${.1} ${u_width}`;
        }
...
/**|--------------------------------------------------------------------------------|
 * |------------------------------------TO-DO---------------------------------------|
 * |------need to get the data from url (for this need to ask sanjiv fro the data)--|
 * |------getting and changing the texture of the rug according to users input------|
 * |--------------------------------------------------------------------------------|*/
    ...
    if (rug_type === "handtufted") {
        ...
        if (material_type === "tibetan_wool" || material_type === "new_zealand_wool") {
            ...
            if (material.pbrMetallicRoughness) {
                ...
                material.pbrMetallicRoughness.metallicRoughnessTexture.setTexture(roughtness_texture);
                material.occlusionTexture.setTexture(oc_texture);
                /**---getting the texture uv's of the */
                nor_sampler = material.normalTexture.texture.sampler;
                mr_sampler = material.pbrMetallicRoughness.metallicRoughnessTexture.texture.sampler;
                oc_sampler = material.occlusionTexture.texture.sampler;
    ...
    modelViewerTexture1.addEventListener("load", async () => {
        ...
        *--|-------------------------------------------------------------------------------|*/
        ...
        try {
            ...
            //no need for oc map//const oc_texture = await modelViewerTexture1.createTexture("Material_Texture/Carpet013_4K_Displacement1_oc.png");
            ...
            if (back_material.pbrMetallicRoughness) {
                ...
                //no need for oc map//back_material.occlusionTexture.setTexture(oc_texture);
