...
async function createTiledTexture(originalImageURL, widthTilingFactor = 2, heightTilingFactor = 2) {
    ...
    // Set canvas size based on tiling factors
    canvas.width = originalImg.width * widthTilingFactor;
    canvas.height = originalImg.height * heightTilingFactor;
...
async function get_set_MainTexture(rug_type, material_type, modeviewer, material, textureURL) {
    const totalSteps = 8; // Number of steps for progress tracking
    let currentStep = 0;
    if (rug_type === "handtufted") {
        ...
        if (material_type === "tibetan_wool" || material_type === "new_zealand_wool" || material_type==="pure_wool") {
            ...
            updateProgress(++currentStep, totalSteps);
            //const nm_texture= await createTiledTexture("Defalut_Maps/silk/Silk_nor.png", 3, 3);
            //const rm_texture= await createTiledTexture("Defalut_Maps/silk/Silk_mr.png", 3, 3);
            //const oc_m_texture= await createTiledTexture("Defalut_Maps/silk/Silk_oc.png", 3, 3);
