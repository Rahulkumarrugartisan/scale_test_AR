...
async function iOS_setMainTexture(rug_type, material_type, modeviewer, material, textureURL) {
    if (rug_type === "handtufted") {
        if (material_type === "tibetan_wool" || material_type === "new_zealand_wool" || material_type === "pure_wool") {
            ...
            const normal_texture = await modeviewer.createTexture("Defalut_Maps/wool/NormalMap (8).png");

            const roughtness_texture = await modeviewer.createTexture("Defalut_Maps/wool/DisplacementMap.png");

            const oc_texture = await modeviewer.createTexture("Defalut_Maps/wool/DisplacementMap.png");
