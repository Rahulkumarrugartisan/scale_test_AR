...
function trackScale(scale, delay = 500) {
    ...
    function checkScale() {
        ...
        if (previousScale !== null && currentScale !== previousScale) {
            // Clear any previous timeout if scaling is detected again
            clearTimeout(scaleTimeout);

            // Set scaling to true immediately
            isScaling = true;

            // Set a timeout to reset isScaling to false after the delay
    ...
    // Return the checkScale function so it can be called periodically
    return checkScale;
}
