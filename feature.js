...
async function iOS_setMainTexture(rug_type, material_type, modeviewer, material, textureURL) {
    if (rug_type === "handtufted") {
        ...
        if (material_type === "tibetan_wool" || material_type === "new_zealand_wool" || material_type === "pure_wool") {
            ...
            const normal_texture = await modeviewer.createTexture("Defalut_Maps/wool/final_wool_iOS_nor.png");

            const roughtness_texture = await modeviewer.createTexture("Defalut_Maps/wool/SpecularMap.png");

            const oc_texture = await modeviewer.createTexture("Defalut_Maps/wool/final_wool_iOS_AO.png");
...
async function get_set_backTexture(rug_type, modelviewer, material, textureURL, shape_param) {
    if (rug_type === "handtufted") {
        ...
        if (shape_param !== "arch" && shape_param !== "capsule" && shape_param !== "drop" && shape_param !== "ogee" && shape_param !== "oval" && shape_param !== "diamond" && shape_param !== "splash" && shape_param !== "hexagon" && shape_param !== "oblong" && shape_param !== "oval" && shape_param !== "round" && shape_param !== "eight" && shape_param !== "halfmoon") {
...
setInterval(() => {
    ...
    info_interaction.textContent = `---isScale: ${is_sacling} scale:: ${modelViewer.cModelScale.x} DF: ${default_tilling} CF: ${current_tilling}`;
